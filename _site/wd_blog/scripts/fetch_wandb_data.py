...
# Parameter norm - larger tick font
fig.update_yaxes(
    title_text="Norm", 
    row=1, col=1, 
...
# Gradient norm - larger tick font with fixed range
fig.update_yaxes(
    title_text="Norm", 
    row=1, col=2, 
    gridcolor='rgba(229, 231, 235, 0.5)',
...
# Loss - fixed range for better visualization with larger tick font
fig.update_yaxes(
    title_text="Loss", 
    row=1, col=3, 
    gridcolor='rgba(229, 231, 235, 0.5)',
...
# Save as HTML
output_file = "wandb_metrics_plot.html"
fig.write_html(
    output_file,
    config={
        'displayModeBar': True,
...
print(f"✓ X-axis label added (larger font)")
print(f"✓ Subplot titles preserved")
print(f"✓ Y-axis tick labels enlarged (size 13)")

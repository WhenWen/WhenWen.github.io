...
for layer in range(num_layers):
    for proj in projections:
        ...
        key = f"grad/norm/transformer.layers.{layer}.self_attn.{proj}_proj.weight"
        metrics.append(key)
...
fig.update_layout(
    title={
        'text': f"Gradient Norms over {num_layers} Layers (Q, K, V)",
        'x': 0.5,
        'xanchor': 'center',
...
# Save as HTML
output_file = "wandb_qkv_grad_norms.html"
fig.write_html(
    output_file,
    config={
        'displayModeBar': True,
